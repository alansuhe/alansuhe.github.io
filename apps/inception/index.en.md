---
docId: inception-index
fileId: index
lang: en
title: Self Inception
description: 
---
{% include lang-selector.md lang=page.lang file=page.fileId %}

## Contact Us
If you have any technical or usage issues, feel free to contact us.
[Email](email:tosuhe@gmail.com)

## Psychological Suggestion Companion
Through regular positive psychological suggestions, cultivate positive thinking patterns and achieve inner growth.

### App Features:
- Custom positive affirmation statements with optional scheduled playback
- Multiple voice styles available, find the voice that suits you best
- Support for multi-language content
- Set daily practice reminders
- Clean, distraction-free interface design

### Usage Scenarios:
- Daily mindfulness practice, building positive self-dialogue
- Pre-sleep psychological preparation, achieving better results in relaxed state
- Enhance psychological resilience, cultivate growth mindset

Let positive psychological suggestions become your daily habit, achieving inner growth in peace.

## Privacy
- No registration required, ready to use upon installation.
- No personal information is collected.
- Speech recognition is processed on the device to protect your privacy.